...
function Room(kurento, options) {
	...
	localParticipant = new Participant(kurento, true, that, {
		id: options.user
	});
		...
		pc.setRemoteDescription(answer, function() {
			...
			if(!local || that.displayMyRemote()) {
				...
				if(wrStream != undefined) {
					speechEvent = kurentoUtils.WebRtcPeer.hark(wrStream, {
						threshold: that.room.thresholdSpeaker
					});
...
function KurentoRoom(wsUri, callback) {
	...
	function initJsonRpcClient() {
		...
		var config = {
			...
			rpc: {
				...
				ctrroomVideo: onCtrroomVideo,
				ctrroomVoice: onCtrroomVoice,
				ctrroomHandsup: onCtrroomHandsup,
				ctrroomKickout: onCtrroomKickout,
				ctrroomNotice: onCtrroomNotice
			}
		};
	...
	function disconnectCallback() {
		...
		if(isRoomAvailable()) {
			...
			alert('Connection error. Please reload page1.');
		}
	}
	...
	function reconnectingCallback() {
		...
		if(isRoomAvailable()) {
			...
			alert('Connection error. Please reload page2.');
		}
	}
	 ...
	 * @param {Object} params
	 */
	function onCtrroomVideo(params) {
		if(isRoomAvailable()) {
			onOffVideocam(params);
		}
	}
	/**
	 * 音频控制
	 * @param {Object} params
	 */
	function onCtrroomVoice(params) {
		if(isRoomAvailable()) {
			onOffVolume(params)
		}
	}
	 ...
	 * @param {Object} params
	 */
	function onCtrroomHandsup(params) {
		//TODO controller提供举手动作提示
	 ...
	 * @param {Object} params
	 */
	function onCtrroomKickout(params) {
		if(isRoomAvailable()) {
			leaveRoom(params);
		}
	}
	...
	//接收会控处理结果通知
	function onCtrroomNotice(params) {
		//TODO 操作成功则修改与会者状态
		return;
	}

	function onOffVolume(params) {
		var element = document.getElementById("buttonVolume");
		var doaction = params.doaction;

		if(params.doaction === 'startvoice' && element.classList.contains("md-volume-off")) { //系统要求打开语音
			element.click();
		} else if(params.doaction === 'stopvoice' && element.classList.contains("md-volume-up")) { //系统要求关闭语音
			element.click();
		} else if(params.doaction === 'startvoice' && element.classList.contains("md-volume-up")) { //系统要求打开语音
			
		} else if(params.doaction === 'stopvoice' && element.classList.contains("md-volume-off")) { //系统要求关闭语音
			
		} else {
			console.log("[会控操作]音频控制操作失败,非法指令");
			return;
		}

		console.log("Sending volume to all");
		this.sendRequest("ctrroomNotice", {
			doaction: doaction
		}, function(error, response) {
			if(error) {
	...
	function onOffVideocam(params) {
		var element = document.getElementById("buttonVideocam");
		var doaction = params.doaction;

		if(params.doaction === 'startvideo' && element.classList.contains("md-videocam-off")) { //系统要求打开音频
			element.click();
		} else if(params.doaction === 'stopvideo' && element.classList.contains("md-videocam")) { //系统要求关闭音频
			element.click();
		} else if(params.doaction === 'startvideo' && element.classList.contains("md-videocam")) { //系统要求打开音频
			
		} else if(params.doaction === 'stopvideo' && element.classList.contains("md-videocam-off")) { //系统要求关闭音频
			
		} else {
			console.log("[会控操作]视频控制操作失败,非法指令");
			return;
		}
	...
	function leaveRoom(params) {
		var element = document.getElementById("buttonLeaveRoom");
		var doaction = params.doaction;

		if(params.doaction === 'kickout') { //系统要求离开会议室
			element.click();
		}else {
			console.log("[会控操作]踢出控制操作失败,非法指令");
			return;
		}

		console.log("Sending ctrroomKickout to mine");
		this.sendRequest("ctrroomNotice", {
			doaction: doaction
		}, function(error, response) {
			if(error) {
